...
void main()
{
  ...
  //int sum;
  auto maxSize = 70_000_000;
  auto freeSpace = maxSize - root.size;
  auto reduction = 30_000_000 - freeSpace;

  Node minWorkingDir = root;

  void walk(Node n)
  {
    if (!n.isDir) return;

    if (n.size >= reduction && n.size < minWorkingDir.size)
      minWorkingDir = n;
